...
def _get_train_and_val_dataset_objects(cfg, need_val_dataset=True):
    ...
    if not need_val_dataset:
        print('Ignore val dataset')
...
def get_train_and_val_dataloader(cfg, need_val_data_loader=True):
    r"""Return dataset objects for the training and validation sets.
    ...
    """
    train_dataset, val_dataset = _get_train_and_val_dataset_objects(cfg, need_val_dataset=need_val_data_loader)
    train_data_loader = _get_data_loader(
        cfg, train_dataset, cfg.data.train.batch_size)
    if not need_val_data_loader:
