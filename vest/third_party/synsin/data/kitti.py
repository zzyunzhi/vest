...
class KITTIDataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.getitem(id, id_target)

    def getitem(self, id, id_target):
        ...
        RB = ROT.from_euler('xyz',poseB[0:3]).as_matrix()#as_dcm()
        TA = poseA[3:].reshape(3, 1)
        RA = ROT.from_euler('xyz',poseA[0:3]).as_matrix()#as_dcm()
        T = RA.T.dot(TB-TA)/50.
