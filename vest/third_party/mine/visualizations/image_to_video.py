...
from vest.third_party.mine.synthesis_task import SynthesisTask
from vest.third_party.mine.operations import mpi_rendering, rendering_utils
...
def img_tensor_to_np(img_tensor):
    ...
    if C == 1:
        img_np_HWC_255 = cv2.applyColorMap(img_np_HWC_255, cv2.COLORMAP_PLASMA)
...
class VideoGenerator:
    def __init__(self, synthesis_task, config, logger, img, output_dir, K=None, xyzs=None):
    ...
    def infer_network(self):
        ...
        if self.pt3d_src is None:
            self.pt3d_src = torch.ones((B, 3, N_pt), dtype=torch.float32).cuda()

        src_items = {
            "img": self.img,
            "K": self.K,
            "K_inv": self.K_inv,
            "xyzs": self.pt3d_src, #torch.ones((B, 3, N_pt), dtype=torch.float32)
        }
        tgt_items = {
        ...
        src_pt3d_pxpy = torch.matmul(self.K, self.pt3d_src)  # Bx3x3 * Bx3xN_pt -> Bx3xN_pt
        ...
        self.scale_factor = self.synthesis_task.compute_scale_factor(src_pt3d_disp_syn, src_pt3d_disp)  # B
...
def main():
    ...
    if False:
        ...
        from vest.utils.visualization.view_syn import get_batch
        cfg = get_attr_dict(os.path.join(ROOT, 'configs/datasets/kitti_city.yaml'))
        # print(cfg)
        cfg.data.generator_params.do_color_aug_p = 0
        cfg.data.generator_params.do_flip_p = 0
        data = get_batch(cfg, batch_size=1, sequence_length=3)
        img_np = data['images'][:1, 1].cuda() * 0.5 + 0.5
